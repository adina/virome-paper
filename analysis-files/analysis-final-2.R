...
gt0 <- filter_taxa(all_vlp, function(x) sum(x) > 0, TRUE)
sum(summary(as.data.frame(tax_table(gt0))$t1))
sum(summary(as.data.frame(tax_table(all_bact))$t1))
sum(summary(as.data.frame(tax_table(all_vlp))$t1))
sum(summary(as.data.frame(tax_table(all_ind))$t1))
...
all_77_day <- subset_samples(all, cage_str == "77" & ext_frac == "vlp" & (Age == 0 | Age == 1))
meta_77 <- sample_data(all_77)
meta_77_day <- sample_data(all_77_day)
v_all <- veganotu(all_77)
v_all <- veganotu(all_77_day)
bd <- vegdist(v_all, method="bray")
...
adonis(bd ~ Age, perm=9999, as(sample_data(all_77_day),"data.frame"))

all_77_day <- subset_samples(all, cage_str == "50" & ext_frac == "ind" & (Age == 0 | Age == 43))
meta_77_day <- sample_data(all_77_day)
v_all <- veganotu(all_77_day)
bd <- vegdist(v_all, method="bray")
adonis(bd ~ Age, perm=9999, as(sample_data(all_77_day),"data.frame"))
...
signif_77 <- subset_samples(signif, cage_str == "50")
t<-merge_samples(signif_77, "Age")
d <- psmelt(t)
d$Abundance[d$Abundance == 0] <- NA
d$log_abund <- log2(d$Abundance)
ggplot(d, aes(x=Sample, y=OTU, fill=log_abund)) + geom_tile() + scale_fill_gradient(low="#66CCFF", high="#000033", na.value="white")+theme_bw()+xlab('Experimental Day')+ylab('Assembled Contig ID')
...
#abund_sig_g1 <- abundance[which(rownames(abundance) ==  "11070_5415_VLP" |  rownames(abundance) ==  "11495_11494_IND" |  rownames(abundance) ==  "1168485_95035_IND" |  rownames(abundance) ==  "11895_11894_IND"  |  rownames(abundance) ==  "1546282_100483_IND" |  rownames(abundance) ==  "2052_2051_IND" |  rownames(abundance) ==  "3314_3313_VLP" |  rownames(abundance) ==  "345_344_IND" |  rownames(abundance) ==  "47_46_IND" |  rownames(abundance) ==  "485381_86166_IND" |  rownames(abundance) ==  "8233_8232_IND" |  rownames(abundance) ==  "9619_9618_IND"),]
#abund_sig_g2 <- abundance[which(rownames(abundance) ==  "11392_11391_IND" |  rownames(abundance) ==  "11500_11499_IND" |  rownames(abundance) ==  "11573_11572_IND" |  rownames(abundance) ==  "11577_11576_IND" |  rownames(abundance) ==  "1395_1394_VLP" |  rownames(abundance) ==  "1460_1459_VLP" |  rownames(abundance) ==  "1545310_100359_IND" |  rownames(abundance) ==  "1568425_102182_IND" |  rownames(abundance) ==  "189_188_IND" |  rownames(abundance) ==  "2281_2280_VLP" |  rownames(abundance) ==  "268318_14671_VLP" |  rownames(abundance) ==  "278888_15308_VLP" |  rownames(abundance) ==  "279518_15392_VLP" |  rownames(abundance) ==  "282627_15745_VLP" |  rownames(abundance) ==  "3398_3397_VLP" |  rownames(abundance) ==  "6233_6232_IND" |  rownames(abundance) ==  "81561_11054_VLP" |  rownames(abundance) ==  "8182_8181_IND" |  rownames(abundance) ==  "9169_9168_IND" |  rownames(abundance) ==  "1301_1300_VLP" |  rownames(abundance) ==  "1541036_99683_IND"),]
abund_sig <- abundance[which(rownames(abundance) ==  "11070_5415_VLP" |  rownames(abundance) ==  "11495_11494_IND" |  rownames(abundance) ==  "1168485_95035_IND" |  rownames(abundance) ==  "11895_11894_IND"  |  rownames(abundance) ==  "1546282_100483_IND" |  rownames(abundance) ==  "2052_2051_IND" |  rownames(abundance) ==  "3314_3313_VLP" |  rownames(abundance) ==  "345_344_IND" |  rownames(abundance) ==  "47_46_IND" |  rownames(abundance) ==  "485381_86166_IND" |  rownames(abundance) ==  "8233_8232_IND" |  rownames(abundance) ==  "9619_9618_IND" | rownames(abundance) ==  "11392_11391_IND" |  rownames(abundance) ==  "11500_11499_IND" |  rownames(abundance) ==  "11573_11572_IND" |  rownames(abundance) ==  "11577_11576_IND" |  rownames(abundance) ==  "1395_1394_VLP" |  rownames(abundance) ==  "1460_1459_VLP" |  rownames(abundance) ==  "1545310_100359_IND" |  rownames(abundance) ==  "1568425_102182_IND" |  rownames(abundance) ==  "189_188_IND" |  rownames(abundance) ==  "2281_2280_VLP" |  rownames(abundance) ==  "268318_14671_VLP" |  rownames(abundance) ==  "278888_15308_VLP" |  rownames(abundance) ==  "279518_15392_VLP" |  rownames(abundance) ==  "282627_15745_VLP" |  rownames(abundance) ==  "3398_3397_VLP" |  rownames(abundance) ==  "6233_6232_IND" |  rownames(abundance) ==  "81561_11054_VLP" |  rownames(abundance) ==  "8182_8181_IND" |  rownames(abundance) ==  "9169_9168_IND" |  rownames(abundance) ==  "1301_1300_VLP" |  rownames(abundance) ==  "1541036_99683_IND"),]
signif <- phyloseq(metadata, abund_sig, annotation)
signif_77 <- subset_samples(signif, cage_str == "77" & ext_frac == "vlp")
t<-merge_samples(signif_77, "Age")

temp <- c("11495_11494_IND","1168485_95035_IND","11070_5415_VLP","1546282_100483_IND", "8233_8232_IND","9619_9618_IND","2052_2051_IND","485381_86166_IND","11895_11894_IND","47_46_IND","3314_3313_VLP","345_344_IND","1301_1300_VLP","1541036_99683_IND","11500_11499_IND","2281_2280_VLP","81561_11054_VLP", "11573_11572_IND","8182_8181_IND","6233_6232_IND", "9169_9168_IND","1541036_99683_IND","1460_1459_VLP","279518_15392_VLP","11577_11576_IND","268318_14671_VLP","278888_15308_VLP","11392_11391_IND","1395_1394_VLP", "189_188_IND","3398_3397_VLP","1568425_102182_IND","282627_15745_VLP")


#abund_sig_g2 <- abundance[which(rownames(abundance) ==  "11392_11391_IND" |  rownames(abundance) ==  "11500_11499_IND" |  rownames(abundance) ==  "11573_11572_IND" |  rownames(abundance) ==  "11577_11576_IND" |  rownames(abundance) ==  "1395_1394_VLP" |  rownames(abundance) ==  "1460_1459_VLP" |  rownames(abundance) ==  "1545310_100359_IND" |  rownames(abundance) ==  "1568425_102182_IND" |  rownames(abundance) ==  "189_188_IND" |  rownames(abundance) ==  "2281_2280_VLP" |  rownames(abundance) ==  "268318_14671_VLP" |  rownames(abundance) ==  "278888_15308_VLP" |  rownames(abundance) ==  "279518_15392_VLP" |  rownames(abundance) ==  "282627_15745_VLP" |  rownames(abundance) ==  "3398_3397_VLP" |  rownames(abundance) ==  "6233_6232_IND" |  rownames(abundance) ==  "81561_11054_VLP" |  rownames(abundance) ==  "8182_8181_IND" |  rownames(abundance) ==  "9169_9168_IND" |  rownames(abundance) ==  "1301_1300_VLP" |  rownames(abundance) ==  "1541036_99683_IND"),]
d <- psmelt(t)
d$OTU=factor(d$OTU, levels=temp)
d$OTU[is.na(d$OTU)] <- "1541036_99683_IND"
#d[61,]$OTU <- "1541036_99683_IND"
#d[64,]$OTU <- "1541036_99683_IND"
#d[91,]$OTU <- "1541036_99683_IND"
#d[39,]$OTU <- "1541036_99683_IND"
d$Abundance[d$Abundance == 0] <- NA
d$log_abund <- -log2(d$Abundance)
ggplot(d, aes(x=Sample, y=OTU, fill=log_abund)) + geom_tile() + scale_fill_gradient(low="#66CCFF", high="#000033", na.value="white", name="-log2(Abundance)")+theme_bw()+xlab('Experimental Day')+ylab('Baseline LF VLP Assembled Contig ID')
...
f2 <- ddply(f, .(l3, diet2, Age, ext_frac, cage_str), summarise, MEAN=mean(SUM), SE=sd(SUM)/sqrt(length(SUM)))
f3 <- subset(f2, cage_str == "50" & ext_frac == "total")
